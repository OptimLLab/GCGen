...
class TNNGKLSProblem : public TGKLSProblem
{
protected:
	...
	double Compute(int index, const vector<double>& y) const override 
	{
		...
		{
		case TND:
			value = CalculateNDFunction(y.data()) - mOptimumValue;
			break;
		case TD:
			value = CalculateDFunction(y.data()) - mOptimumValue;
			break;
		case TD2:
			value = CalculateD2Function(y.data()) - mOptimumValue;
		}
...
public:
	...
	TNNGKLSProblem(int problemIndex = 1, int dim = 2, GKLSClass type = Simple, GKLSFuncionType functionType = TD)
		:TGKLSProblem(problemIndex, dim, type, functionType){}
};
